  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      ...
      if (selectedFilters.Category?.length > 0) {
        // Mock filter logic: assume product type is what they meant.
        // We might not have specific product type categorization in product data
      }
    ...
    <div className="min-h-screen">
      ...
      <main>
        <section className="relative h-[50vh] min-h-[400px] overflow-hidden">
          <img src={menImg} alt="Men's collection" className="absolute inset-0 w-full h-full object-cover" />
          ...
          <div className="relative h-full flex items-center justify-center text-center px-4">
            <div className="animate-fade-up">
              ...
              <p className="text-sm tracking-[0.2em] uppercase text-primary-foreground/80 mb-8">
                Modern Essentials. Refined Details.
              </p>
              <Link to="/men" className="btn-primary inline-block bg-primary-foreground text-foreground hover:bg-primary-foreground/90">
                Shop New Arrivals
              </Link>
            </div>
          </div>
